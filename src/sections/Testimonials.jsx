            ...
            <div className='relative max-container min-h-screen max-lg:mt-40 lg:mt-20'>
                ...
                <div className=' relative flex flex-row max-lg:flex-col justify-center items-center min-h-screen'>
                    ...
                    <div className='relative flex flex-row gap-2 max-lg:justify-center max-md:mt-10 lg:top-0 lg:left-72'>
                        <button onClick={handleLeftClick} className='hover:scale-110 transition ease-out duration-500 lg:hidden'><ArrowLeftCircleIcon className='max-sm:h-10 max-sm:w-10 h-20 w-20'/></button>
                        <button onClick={handleRightClick}><ArrowRightCircleIcon className='max-sm:h-10 max-sm:w-10 h-20 w-20 relative left-0'/></button>
                    </div>

                    {/* div 2 */}
                    <div className='flex flex-row relative gap-2 max-sm:p-2 lg:-right-[20%] max-md:top-20 md:top-0 overflow-hidden max-lg:w-full'>
                        {
                            testimonials.map((item, index) => (
                                ...
                                >
                                    <div style={{backgroundImage: `url(${item.img})`}} key={index} className="bg-cover z-10 bg-center relative mx-auto max-md:h-40 max-md:w-40 h-80 w-80 rounded-full"/>

                                    <div className="h-2/3 w-full md:h-[420px] md:w-[500px] border-2 border-black relative bottom-20 rounded-3xl max-md:mx-auto md:left-[26%] lg:left-[33%] 2xl:left-[31%] text-center" >
                                        {/* <div className="p-2 m-10 mt-20 lg:text-center"> */}
                                            ...
                                            <p className="max-sm:text-xs max-lg:text-xl md:font-poppins mt-2 p-4 tracking-widest 2xl:tracking-normal 2xl:text-xl">❝ {item.text} ❞</p>
                                        {/* </div> */}
                                    </div>
                                </div>
                            ))
